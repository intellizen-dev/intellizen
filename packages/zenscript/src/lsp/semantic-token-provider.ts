...
export class ZenScriptSemanticTokenProvider extends AbstractSemanticTokenProvider {
  ...
  private readonly rules: RuleMap = {
    IntegerLiteral: (source, acceptor) => {
      acceptor({ node: source, property: 'value', type: SemanticTokenTypes.number })
    },
    ...
    UnquotedString: (source, acceptor) => {
      acceptor({ node: source, property: 'value', type: SemanticTokenTypes.string })
    },

    ValueParameter: (source, acceptor) => {
      acceptor({ node: source, property: 'name', type: SemanticTokenTypes.parameter, modifier: ['readonly'] })
    },
    ...
    NamedTypeReference: (source, acceptor) => {
      acceptor({ node: source, property: 'path', type: SemanticTokenTypes.class })
      acceptor({ node: source, property: 'typeArguments', type: SemanticTokenTypes.typeParameter })
    },
    ...
    ReferenceExpression: (source, acceptor) => {
      switch (source.target.ref?.$type) {
        ...
        case 'SyntheticHierarchyNode':
          acceptor({ node: source, property: 'target', type: SemanticTokenTypes.namespace })
          break

        case 'ClassDeclaration':
          acceptor({ node: source, property: 'target', type: SemanticTokenTypes.class })
          break
        ...
        case 'VariableDeclaration': {
          acceptor({
            ...
            modifier: READONLY_PREFIX.includes(source.target.ref.prefix) ? 'readonly' : undefined,
          })
          break
        }

        case 'FunctionDeclaration': {
          acceptor({ node: source, property: 'target', type: SemanticTokenTypes.function })
          break
        }

        case 'LoopParameter':
          acceptor({ node: source, property: 'target', type: SemanticTokenTypes.parameter, modifier: 'readonly' })
          break
    ...
    MemberAccess: (source, acceptor) => {
      switch (source.target.ref?.$type) {
        ...
        case 'FieldDeclaration':
          acceptor({ node: source, property: 'target', type: SemanticTokenTypes.property })
          break
    ...
    FunctionDeclaration: (source, acceptor) => {
      acceptor({ node: source, property: 'name', type: SemanticTokenTypes.function })
    },

    ClassDeclaration: (source, acceptor) => {
      acceptor({ node: source, property: 'name', type: SemanticTokenTypes.class })
    },

    FieldDeclaration: (source, acceptor) => {
      acceptor({ node: source, property: 'name', type: SemanticTokenTypes.property })
    },
    ...
    VariableDeclaration: (source, acceptor) => {
      acceptor({
        ...
        property: 'name',
        type: SemanticTokenTypes.variable,
        modifier: READONLY_PREFIX.includes(source.prefix) ? 'readonly' : undefined,
      })
    },
  }
}
