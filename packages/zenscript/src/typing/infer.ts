...
import { isArrayLiteral, isTypeReference, isMapLiteral, isNullLiteral, isNumberLiteral, isStringTemplate, isArrayType, isAssignment, isBooleanLiteral, isBracketExpression, isConditionalExpression, isExpression, isFunctionExpression, isFunctionType, isInfixExpression, isInstanceofExpression, isIntersectionType, isListType, isLiteralExpression, isMapType, isParenthesizedExpression, isParenthesizedType, isPrefixExpression, isPrimitiveType, isReferenceType, isStringLiteral, isTypeCastExpression, isUnionType, isLocalVariable, isClassDeclaration, isVariableDeclaration } from '../generated/ast'

import { ClassTypeDescription, type TypeDescription } from './description'

import { createAnyType, createArrayType, createClassType, createFunctionType, createListType, createMapType, createPrimitiveType } from './factory'
...
export class ZenScriptTypeInferrer implements TypeInferrer {
    ...
    private inferLiteralExpressionType(node: LiteralExpression): TypeDescription | undefined {
        if (isArrayLiteral(node)) {
            const elementType = node.values.length > 0
                && this.inferExpressionType(node.values[0])
                || createAnyType()
        ...
        if (isMapLiteral(node)) {
            const keyType = node.entries.length > 0
                && this.inferExpressionType(node.entries[0].key)
                || createAnyType()
            const valueType = node.entries.length > 0
                && this.inferExpressionType(node.entries[0].value)
                || createAnyType()
            return createMapType(keyType, valueType)
        }
